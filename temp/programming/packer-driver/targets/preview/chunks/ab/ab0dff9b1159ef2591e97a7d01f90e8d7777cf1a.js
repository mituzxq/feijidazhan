      ...
      _export("Missile", Missile = (_dec = ccclass('Missile'), _dec(_class = class Missile extends Component {
        ...
        update(deltaTime) {
          ...
          if (Math.sqrt(Math.pow(this.node.x - this.targetPos.x, 2) + Math.pow(this.node.y - this.targetPos.y, 2)) > this.moveLimit) {
            ...
            var _targetAngle = targetRadian * 180 / Math.PI; //导弹当前角度


            var curAngle = this.node.angle; //导弹当前角度和导弹最终指向角度的差

            var distAngle = _targetAngle - curAngle; //导弹转动的角速度，根据角度差来判断顺时针或逆时针。( stepAngle=null时才会计算旋转角度，为了防止导弹出现在边界值附近左右摇摆的问题，一旦确定旋转角度方向，则不再改变)
            ...
            if (Math.abs((curAngle + 360) % 360 - (_targetAngle + 360) % 360) > Math.abs(this.stepAngle) + 1) {
              //角度未达到临近值才会++，防止抖动
              if (Math.abs(this.node.angle - _targetAngle) > Math.abs(this.stepAngle) + 1) {
        ...
        rotation() {
          if (Math.sqrt(Math.pow(this.node.position.x - this.node.parent.getChildByName("enemy").position.x, 2) + Math.pow(this.node.position.y - this.node.parent.getChildByName("enemy").position.y, 2)) > this.moveLimit) {
            ...
            var distAngle = targetAngle - curAngle;
...
//# sourceMappingURL=ab0dff9b1159ef2591e97a7d01f90e8d7777cf1a.js.map