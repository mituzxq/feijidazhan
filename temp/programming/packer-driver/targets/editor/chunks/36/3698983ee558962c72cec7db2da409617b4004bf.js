      ...
      _export("Missile", Missile = (_dec = ccclass('Missile'), _dec(_class = class Missile extends Component {
        start() {
          this.schedule(() => {
            ...
            let nodeX = node.x;
            let nodeY = node.y;
            let birY = this.node.parent.getChildByName("enemy").getComponent(_crd && Enemy === void 0 ? (_reportPossibleCrUseOfEnemy({
              error: Error()
            }), Enemy) : Enemy).node.position.x;
            let birX = this.node.parent.getChildByName("enemy").getComponent(_crd && Enemy === void 0 ? (_reportPossibleCrUseOfEnemy({
              error: Error()
            }), Enemy) : Enemy).node.position.y;
...
//# sourceMappingURL=3698983ee558962c72cec7db2da409617b4004bf.js.map